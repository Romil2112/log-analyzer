...
def _mark(labels: dict, ip: str, cls: str, malicious: bool, **extra) -> None:
    labels[ip] = {"class": cls, "malicious": malicious, **extra}


def _emit_brute_force(lines: list, labels: dict) -> None:
    ...
    for ip, n in [("185.220.101.10", 40), ("45.33.32.156", 25), ("194.165.16.77", 15)]:
        _mark(labels, ip, "brute_force", True, note="dense burst, many usernames, no success")
...
def _emit_port_scan(lines: list, labels: dict) -> None:
    for ip, n in [("203.0.113.42", 30), ("198.51.100.100", 25)]:
        _mark(labels, ip, "port_scan", True, note="many unique source ports, single burst")
...
def _emit_slow_stuffer(lines: list, labels: dict) -> None:
    ...
    _mark(labels, stuffer, "anomaly", True, note="slow stuffer, below rule threshold, ML-only")
    t = BASE
    for _ in range(30):
...
def _emit_benign_gateway(lines: list, labels: dict) -> None:
    gw = "172.16.0.10"
    _mark(labels, gw, "benign", False, note="corporate gateway, only successful logins")
...
def _emit_noise(lines: list, labels: dict) -> None:
    """Background IPs with a handful of failed logins, always below threshold."""
    for i in range(15):
        ip = f"10.10.{i}.5"
        _mark(labels, ip, "benign", False, note="occasional failed login, below threshold")
        for _ in range(random.randint(1, 4)):
            t = BASE + timedelta(minutes=random.randint(0, 120))
            lines.append((t, _failed(t, random.choice(USERNAMES), ip)))


def _emit_fp_trap_fatfinger(lines: list, labels: dict) -> None:
    ...
    trap_a = "172.16.9.9"
    _mark(labels, trap_a, "benign", False, fp_trap=True,
...
def _emit_fp_trap_internal(lines: list, labels: dict) -> None:
    ...
    trap_b = "10.0.0.9"
    _mark(labels, trap_b, "benign", False, fp_trap=True, allowlist_candidate=True,
...
# Phases run in this fixed order so the seeded RNG stream — and therefore the
# generated corpus — stays byte-for-byte reproducible.
_PHASES = (
    _emit_brute_force, _emit_port_scan, _emit_slow_stuffer, _emit_benign_gateway,
    _emit_noise, _emit_fp_trap_fatfinger, _emit_fp_trap_internal,
)
